...
/// rumman
//#include "/home/sigmind/.HTPA-KK-1/DS_sources/gst-plugins/gst-dsdirection/dsdirection_lib/dsdirection_lib.h"
//#include "/home/sigmind/.HTPA-KK-1/DS_sources/gst-plugins/gst-dsexample/dsexample_lib/dsexample_lib.h"
#include "./DS_sources/gst-plugins/gst-dsexample/dsexample_lib/dsexample_lib.h"
...
static std::vector< watchcam::rectangle > rects, cut_rects, inside_rects; // display size in rectangle
static QVector<QVector<QPair<int, int>> > vect;
...
guint lp_count_checker = 0;//johurul
...
void Deepstream_source_code::constructorDeepStream(){
    ...
    if(connectionDB()){
        ...
        if (query.exec(
                    "CREATE TABLE SmartRecordLog ( "
                    "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                    "session_id int NOT NULL, "
                    "source_id int NOT NULL,"
                    "record_duration VARCHAR(20) NOT NULL, "
            ...
            qDebug()<< "SmartRecordLog Table created successfully";
        }
        else{
            qDebug()<< "SmartRecordLog Table created failed!";
        }
    }
    ...
    char *array[] = {
        ...
        //        "/home/sigmind/Pictures/sabbir/car_wiht_lp_and_recogFace_with_DBFace_branch_bkup/.HTPA-KK-1/.cfg/deepstream-app/blablabla.txt",
    ...
    for (i = 0; i < num_instances; i++) {
        ...
        for (j = 0; j < appCtx[i]->config.num_sink_sub_bins; j++) {
            ...
            QList<float> arra;
            arra.append(1);arra.append(1);arra.append(2);arra.append(3);arra.append(4);

            coordinatesOfFrame(appCtx[i]->config.tiled_display_config.rows, appCtx[i]->config.tiled_display_config.columns, width, height, arra, appCtx[i]);

            g_print("Width of mainWindow %d\n", MainWindow::getDispWidth());
            g_print("Height of mainWindow %d\n", MainWindow::getDispHeight());


            /// backup recent [ 4 vager 3 vag fillup kore]
            //            windows[i] =
            //                    XCreateSimpleWindow (display, RootWindow (display,
            //                                                              DefaultScreen (display)), 0, 0, (MainWindow::getDispWidth()/2 + MainWindow::getDispWidth()/4)-50 , MainWindow::getDispHeight()/2, 2, 0x00000000,
            //                                         0x00000000);

            windows[i] =
                    XCreateSimpleWindow (display, RootWindow (display,
                                                              DefaultScreen (display)), 0, 0, (MainWindow::getDispWidth()/2)-50+50 , MainWindow::getDispHeight()/2, 2, 0x00000000,
                                         0x00000000);
...
void Deepstream_source_code::gotDivisorValue(std::vector<std::pair<int, double>> slider_value_list)//always keep update line
{
    //    qDebug() << "Static ..............."<< rects.size();
    for(int ij = 0; ij < slider_value_list.size(); ij++){
        for(int i = 0; i < rects.size(); i++){
            //            qDebug() << "slider_value_list[ij].first = " << slider_value_list[ij].first;
            if(i == slider_value_list[ij].first){
                int l = rects[i].left;
                int t = rects[i].top;
                int r = rects[i].right;
                int b = rects[i].bot;
                watchcam::rectangle rect(l, (((b-t)*slider_value_list[ij].second)/10)+t, r, (((b-t)*slider_value_list[ij].second)/10)+t);
                //                qDebug() << "cASSS1 "<< cut_rects[ij].left << " " << cut_rects[ij].top;
                cut_rects[ij] = rect;
                //                qDebug() << "cASSS "<< cut_rects[ij].left << " " << cut_rects[ij].top;
            }
        }
    }
...
//std::vector<int> Deepstream_source_code::getSourceVecInSettings()
//{
//    std::vector<int> source_enable_or_not;
//    for(int s= 0; s < appCtx[0]->config.tiled_display_config.rows*appCtx[0]->config.tiled_display_config.columns/*appCtx[i]->config.num_source_sub_bins*/; s++){
//        source_enable_or_not.push_back(appCtx[0]->config.multi_source_config[s].camera_id);
//    }
//    return source_enable_or_not;
//}
...
void Deepstream_source_code::process_meta (AppCtx * appCtx, NvDsBatchMeta * batch_meta)
{
    ...
    NvDsConfig *config = &appCtx->config;
    NvDsSourceConfig * configs = config->multi_source_config;

    // GList *rect_list = g_hash_table_get_values(configs->rect_params_table);

    //    for(NvSOURCE_RectParams *based = (NvSOURCE_RectParams*)rect_list->data;
    //        rect_list != NULL; rect_list = rect_list->next){
    //        qDebug() << "got it1 " << based[1].x0;
    //        qDebug() << "got it " << based->x0;
    //    }

    // while(rect_list != NULL){
    //NvSOURCE_RectParams *based = (NvSOURCE_RectParams*)rect_list->data;
    // based->x0; based->y0;
    //  qDebug()<<"process meta x0: "<<based->x0<<" y0: "<<based->y0<<" "<<based->x1<<" "<<based->y1<<" "<<based->x2<<" "<<based->y2<<" "<<based->x3<<" "<<based->y3;
    //rect_list = rect_list->next;
    // }
    //    qDebug() << "we can see " << configs->rect_params.x0;
    //    qDebug() << "start "<< config->num_source_sub_bins;
    for(int k = 0; k < cut_rects.size(); k++){
        //qDebug()<<"tester b "<<configs[k].source_id;

        if (!configs[k].enable) {
            continue;
        }
        //                qDebug() << "Divisors value " << configs[k].divisor;
        //        qDebug() << rects[k].left << " "<< rects[k].top << " "<< rects[k].right << " "<< rects[k].bot;
        NvDsDisplayMeta *display_meta;
        display_meta = nvds_acquire_display_meta_from_pool (batch_meta);
        ...
        //        line_params[0].x1 = cut_rects[k].left;
        //        line_params[0].y1 = cut_rects[k].top;
        //        line_params[0].x2 = cut_rects[k].right;
        //        line_params[0].y2 = cut_rects[k].bot;
        //        line_params[0].line_width = 5;
        //        line_params[0].line_color = (NvOSD_ColorParams){1.0, 0.0, 0.0, 0.5};


        //NvOSD_RectParams *sub_rect_params  = &display_meta->rect_params_j[0];// for testing
        display_meta->num_rects = 1;


        //        sub_rect_params[0].left = (float)inside_rects[0].left;
        //        sub_rect_params[0].top = (float)inside_rects[0].top;
        //        sub_rect_params[0].width = (float)inside_rects[0].right;
        //        sub_rect_params[0].height = (float)inside_rects[0].bot;
        //        sub_rect_params[0].border_width = 5;
        //        sub_rect_params[0].border_color = (NvOSD_ColorParams){1.0, 0.0, 0.0, 0.5};

        //        qDebug()<<"inside_rects[0].left: "<<sub_rect_params[0].left<<" inside_rects[0].top: "<<sub_rect_params[0].top
        //               <<" inside_rects[0].right: "<<sub_rect_params[0].width<<" inside_rects[0].bot: "<<sub_rect_params[0].height;
...
{
    gulong dsanalytics_probe_id=26;
    gboolean ret = FALSE;
    *sink_elem = *src_elem = NULL;
    ...
    if (config->dsanalytics_config.enable) {
        ...
        NVGSTDS_ELEM_ADD_PROBE (dsanalytics_probe_id,
                                pipeline->common_elements.dsanalytics_bin.bin, "src",
                                Deepstream_source_code::nvdsanalytics_src_pad_buffer_probe, GST_PAD_PROBE_TYPE_BUFFER,
                                appCtx);
...
void Deepstream_source_code::coordinatesOfFrame(int rrows, int ccolumns, int w, int h, QList<float> divisible, AppCtx * appCtx)
{
    NvDsConfig *config = &appCtx->config;
    NvDsSourceConfig * configs = config->multi_source_config;
    //    qDebug() << "start "<< config->num_source_sub_bins;
    //    qDebug() << "inside coordinatesOfFrame() method\n rows : " << rrows << " colomn: " << ccolumns<<" width: "<<w<<" height: "<<h<<endl;

    int source_index=0;

    int x = 0;
    int y = 0;
    int single_width = w/ccolumns;
    int single_column = h/rrows;
    qDebug() << "single_width : " << single_width << " and single_column: " << single_column<<"\n";
    ...
    //            vt.push_back(qMakePair(x_, y_));// single feed push here
    ////            qDebug() << "x_ : " << x_<<" and y_ : "<<y_<<"\n";
    //            x_ += single_width;
    //        }
    //        x_ = x;
    //        y_ += single_column;
    //        vect.push_back(vt);//full row push here
    ////        qDebug() << "vt size : " << vt.size()<<" and vect size : "<<vect.size()<<"\n";
    //    }
    //    for(int i = 1; i < vect.size(); i++){
    //        for(int j = 1; j < vect[i].size(); j++){
    //            // take (left,top, right, bottom)
    //            watchcam::rectangle rect(vect[i-1][j-1].first, vect[i-1][j-1].second, vect[i-1][j].first, vect[i][j].second);//feed rect create
    ////            qDebug() << "x0 : " << vect[i-1][j-1].first <<" y0 : "<<vect[i-1][j-1].second
    ////                    <<" \nand x1 : "<<vect[i-1][j].first<<" y1 : "<<vect[i][j].second<<"\n";

    //            rects.push_back(rect);// push each feed window here

    //            //qDebug() << "divisor value from config : " << configs[source_index].divisor;

    ////            watchcam::rectangle rect_cut(vect[i-1][j-1].first,
    ////                    ((vect[i][j-1].second-vect[i-1][j-1].second)/configs[source_index].divisor)+vect[i-1][j-1].second,
    ////                    vect[i-1][j].first,
    ////                    ((vect[i][j].second-vect[i-1][j].second)/configs[source_index].divisor)+vect[i-1][j].second);

    //            // design custom algorithm here
    ////            watchcam::rectangle rect_cut(vect[i-1][j-1].first,
    ////                    (((vect[i][j-1].second-vect[i-1][j-1].second)*configs[source_index].divisor)/10)+vect[i-1][j-1].second,
    ////                    vect[i-1][j].first,
    ////                    (((vect[i][j].second-vect[i-1][j].second)*configs[source_index].divisor)/10)+vect[i-1][j].second);//line left, top, right , bottom

    //          //  ++source_index;
    //           // cut_rects.push_back(rect_cut);// draw line with feed push here
    //        }
    //    }
    //    watchcam::rectangle inside_sub_rect(200, 100, 400, 500);//inside rect create
    //    inside_rects.push_back(inside_sub_rect);
    //    qDebug() << "inside rects size : " << inside_rects.size();
...
gboolean Deepstream_source_code::create_pipeline(AppCtx *appCtx, bbox_generated_callback bbox_generated_post_analytics_cb, bbox_generated_callback all_bbox_generated_cb, perf_callback perf_cb, overlay_graphics_callback overlay_graphics_cb)
{
    ...
    if (config->dsexample_config.enable) {
        ...
        NVGSTDS_ELEM_ADD_PROBE (dsexample_probe_id,
                                ...
                                Deepstream_source_code::dsexample_buf_probe, GST_PAD_PROBE_TYPE_BUFFER,
                                appCtx);
        // Set this bin as the last element
...
//GstPadProbeReturn nvdsanalytics_src_pad_buffer_probe (GstPad * pad, GstPadProbeInfo * info,
//    gpointer u_data)
//{
//    qDebug()<<"johurul 1";
//    GstBuffer *buf = (GstBuffer *) info->data;
...
//            if(overcrowding){
//                printf("Overcrowding detected ????????????????????????\n");

//                MainWindow::fromDsToMainForOvercrowding(source_id);
//            }
//        }
...
GstPadProbeReturn Deepstream_source_code::nvdsanalytics_src_pad_buffer_probe (GstPad * pad, GstPadProbeInfo * info,
                                                                              gpointer u_data)
{
    GstBuffer *buf = (GstBuffer *) info->data;
    guint num_rects = 0;
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        for (l_obj = frame_meta->obj_meta_list; l_obj != NULL;
            ...
            for (NvDsMetaList *l_user_meta = obj_meta->obj_user_meta_list; l_user_meta != NULL;
                ...
                if(user_meta->base_meta.meta_type == NVDS_USER_OBJ_META_NVDSANALYTICS)
                {
                    ...
                    g_print("\nuser_meta_data->lcStatus sz : %d\n", user_meta_data->lcStatus.size());
                    cout<<"\nuser_meta_data->lcStatus : "<<user_meta_data->lcStatus[0]<<endl;
                    //                    g_print("\nuser_meta_data->lcStatus : %s\n", user_meta_data->lcStatus[0]);
        ...
        for (NvDsMetaList * l_user = frame_meta->frame_user_meta_list;
            ...
            //                               MainWindow::fromDsToMainForOvercrowding(lc_count);
            //            }
            //                printf("Overcrowding detected ????????????????????????\n");
...
/// for modify work only pgie [fd_lpd.txt]
...
GstPadProbeReturn Deepstream_source_code::dsexample_buf_probe(GstPad *pad, GstPadProbeInfo *info, gpointer u_data)
{
    ...
    for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        for (NvDsMetaList * l_user = frame_meta->frame_user_meta_list;
            ...
            // Get the labels from nvdsanalytics config file
            //roi_count = meta->objInROIcnt["RF"];
            lc_count_lp = meta->objLCCumCnt["Exit"];
            //            if(lp_count_checker == 0){//store first value

            //                lp_count_checker = lc_count_lp;
            //                qDebug()<<"first store value : "<<lp_count_checker;

            //            }
            //overcrowding = meta->ocStatus["OC"];

            printf("LC  detected lp = %d\n", lc_count_lp);
        ...
        for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL;
            ...
            gchar white[10] = "white";
            gchar red[5] = "red";

            //            qDebug()<<"obj_meta->unique_component_id is : "<<obj_meta->unique_component_id<<endl;


            int obj_class_id = (int)obj_meta->class_id;
            int obj_track_id;
            if(obj_class_id == 0){// 0 means Car, 1=Bycycle, 2=Person, 3=roadsign
                obj_track_id = (int)obj_meta->object_id;
            }
            qDebug()<<"object_id : "<<obj_meta->object_id<<" fstclass_id: "<<obj_meta->class_id<<" obj_label : "<<obj_meta->obj_label<<endl;
            ...
            qDebug()<<"before lp loop";

            for (NvDsMetaList * l_user_meta = obj_meta->obj_user_meta_list; l_user_meta != NULL;
                 l_user_meta = l_user_meta->next){

                NvDsUserMeta * user_meta = (NvDsUserMeta *) (l_user_meta->data);
                qDebug() << "user_meta->base_meta.meta_type : "<<user_meta->base_meta.meta_type<<" "<<NVDS_USER_FRAME_META_EXAMPLE;

                if(user_meta->base_meta.meta_type == NVDS_USER_FRAME_META_EXAMPLE){

                    DsExampleOutput* dsexample_user_meta = (DsExampleOutput*)user_meta->user_meta_data;
                    cv::Mat *metadata = dsexample_user_meta->matrix;

                    //                                    cv::Mat *metadata = NULL;
                    //                                    metadata = (cv::Mat*) user_meta->user_meta_data;//sub object face/licence plate

                    std::string path = "/home/sigmind/Desktop/lp/" ;
                    if(metadata->rows > 0 && metadata->cols > 0){
//                                            cv::imwrite(path+std::to_string(frame_meta->frame_num) +"_1"+ ".jpeg" ,*metadata_one);
//                                            cv::imwrite(path+std::to_string(frame_meta->frame_num)+"_2" + ".jpeg" ,*metadata);
                    ...
                    watchcam::rectangle rect(rectL,rectT,rectW,rectH);
                    rect.type = obj_meta->class_id;
                    qDebug()<<"rct typeee "<<rect.type;

//                    rect.str_color =((std::string) str_colr);
                    rect.source_id = source_id;
                    rect.fps = val;
                    rect.track_id = obj_track_id;

                    qDebug()<<"lp_count_checker : "<<lp_count_checker<<" lc_count_lp : "<<lc_count_lp;
                    if(lp_count_checker < lc_count_lp){
                        qDebug()<<"lp_count_checker inside: "<<lp_count_checker<<" lc_count_lp : "<<lc_count_lp;
                        lp_count_checker = lc_count_lp;
                        std::string path = "/home/sigmind/Desktop/man/" ;
                        //                            cv::imwrite(path+std::to_string(frame_meta->frame_num) +"_face"+ ".jpeg" ,*metadata_face);
//                                            if((metadata->rows > 0 && metadata->cols > 0) && (metadata_one->rows > 0 && metadata_one->cols > 0)){
//                                                MainWindow::fromDsToMain(metadata_one->clone(), metadata->clone(), rect);
//                                            }
//                                            if(metadata->rows > 0 && metadata->cols > 0){
//                                                MainWindow::fromDsToMain(metadata->clone(), metadata->clone(), rect);
//                                            }
                    }

                    //                                        qDebug()<<"metadata_one->rows : "<<metadata_one->rows<<" metadata_one->cols : "<<metadata_one->cols;
                    if(metadata->rows > 0 && metadata->cols > 0){
                        MainWindow::fromDsToMain(metadata->clone(), metadata->clone(), rect);
                    }
                    //                                    MainWindow::fromDsToMain(metadata->clone(), rect);
...
//                    qDebug()<<"secondary gie_config->enable : "<<(gint) gie_config->enable
//                           <<" operate_on_gie_id : "<<gie_config->operate_on_gie_id<<" gie unique-id is : "<<gie_config->unique_id
//                          <<" obj_meta->unique_component_id : "<<obj_meta->unique_component_id;
//                    //                    qDebug()<<"secondary gie unique-id is : "<<(gint) gie_config->unique_id;

//                    if (obj_meta->unique_component_id == (gint) gie_config->unique_id) {

//                        ///for pgie
////                        for (NvDsMetaList * l_user_meta = obj_meta->parent->obj_user_meta_list; l_user_meta != NULL;
////                             l_user_meta = l_user_meta->next){

////                            qDebug() << "pgie user meta";
////                            NvDsUserMeta * user_meta = (NvDsUserMeta *) (l_user_meta->data);

////                            qDebug()<<"user meta type : "<<user_meta->base_meta.meta_type;

////                            DsExampleOutput* dsexample_user_meta_main = (DsExampleOutput*)user_meta->user_meta_data;
////                            metadata_one = dsexample_user_meta_main->matrix;
////                            //                            DsExampleObject* user_obj_meta = dsexample_user_meta_main->object;
////                            //                            qDebug()<<"user_obj_meta : "<<user_obj_meta->label; /// sabbir changed

////                            qDebug()<<"object_id : "<<obj_meta->object_id<<" sndclass_id: "<<obj_meta->class_id<<" obj_label : "<<obj_meta->obj_label<<endl;


////                            //                            metadata_one = (cv::Mat*) user_meta->user_meta_data;// main object person/car

////                            qDebug()<<"metadata_one->rows : "<<metadata_one->rows<<" metadata_one->cols : "<<metadata_one->cols;

////                            std::string path = "/home/sigmind/Desktop/car/" ;
////                            //                            if(metadata_one->rows > 0 && metadata_one->cols > 0)
////                            //                            cv::imwrite(path+std::to_string(frame_meta->frame_num) + ".jpeg" ,*metadata_one);

////                        }


//                        obj_meta->parent->classifier_meta_list =
//                                g_list_sort (obj_meta->parent->classifier_meta_list, component_id_compare_func);

//                        for (NvDsMetaList * l_class = obj_meta->parent->classifier_meta_list; l_class != NULL;
...
//                            //                                NvDsUserMeta * user_meta = (NvDsUserMeta *) (l_user_meta->data);
//                            //                                qDebug() << "user_meta->base_meta.meta_type : "<<user_meta->base_meta.meta_type;
//                            //                                qDebug()<<"object_id : "<<obj_meta->object_id<<" trdclass_id: "<<obj_meta->class_id<<" obj_label : "<<obj_meta->obj_label<<endl;
...
//                            //                                std::string path = "/home/sigmind/Desktop/lp/" ;
//                            ////                                cv::imwrite(path+std::to_string(frame_meta->frame_num) + ".jpeg" ,*metadata);
...
//                                qDebug() <<  "inner loop j "<<label->pResult_label<<", "<<label->result_label[0];
//                                //                                if (label->pResult_label) {
//                                //                                }
//                                //                                else if (label->result_label[0] != '\0') {

//                                gchar* str_colr = NULL;
//                                str_colr = g_strdup_printf("%s",label->result_label);
//                                qDebug() << "vehicle str color = " << label->result_label;
//                                //strcmp(text, "hello") == 0
//                                if(strcmp(str_colr, "Obj_label") == 0){
//                                    qDebug() << "j con true ";
//                                    str_colr = "Unrecognized";
//                                }
...
//                }
////                try{
////                    metadata_one->release();
////                    metadata_one->deallocate();
////                }catch(...){}
//            }
...
/// for backup and work pgie with sgie also
...
GstPadProbeReturn Deepstream_source_code::dsexample_buf_probe(GstPad *pad, GstPadProbeInfo *info, gpointer u_data)
{
    ...
    for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL;
            ...
            qDebug()<<"outside if and else obj_meta->unique_component_id :"<<obj_meta->unique_component_id<<
                      " appCtx->config.primary_gie_config.unique_id : "<<appCtx->config.primary_gie_config.unique_id;

            if (obj_meta->unique_component_id ==
                    (gint) appCtx->config.primary_gie_config.unique_id) {//car
                gie_config = &appCtx->config.primary_gie_config;

                //                qDebug()<<"pgie obj_meta->unique_component_id : "<<obj_meta->unique_component_id
                //                       <<" appCtx->config.primary_gie_config.unique_id : "<<appCtx->config.primary_gie_config.unique_id;
                ...
                for (gint i = 0; i < (gint) appCtx->config.num_secondary_gie_sub_bins;
                    ...
                    if (obj_meta->unique_component_id == (gint) gie_config->unique_id) {
                        //johir2 fstclass_id:  0  obj_label :  face
//                        qDebug()<<"johir2 fstclass_id: "<<obj_meta->class_id<<" obj_label : "<<obj_meta->obj_label<<endl;
...
//                            qDebug()<<"pgie user meta type : "<<user_meta->base_meta.meta_type;
...
//                                qDebug()<<"object_id : "<<obj_meta->object_id<<" sndclass_id: "<<obj_meta->class_id<<" obj_label : "<<obj_meta->obj_label<<endl;
                                ...
                                //std::string path = "/home/sigmind/Desktop/car_sgie_parent/" ;
                                //if(metadata_one->rows > 0 && metadata_one->cols > 0){
                                    //cv::imwrite(path+std::to_string(frame_meta->frame_num) + ".jpeg" ,*metadata_one);
                                //}
                            }
                        ...
                        for (NvDsMetaList * l_user_meta = obj_meta->obj_user_meta_list; l_user_meta != NULL;
                            ...
                            if(user_meta->base_meta.meta_type == NVDS_USER_FRAME_META_EXAMPLE){
                                ...
                                qDebug()<<"metadata->rows : "<<metadata->rows<<" metadata->cols : "<<metadata->cols;
                                ...
                                //                                        rect.str_color =((std::string) str_colr);
                                rect.str_color =((std::string) "Obj_label");
                                rect.source_id = source_id;
                                rect.fps = val;
                                rect.track_id = obj_meta->object_id;
                                rect.str_label = obj_meta->obj_label;
...
//                                }
                                ...
                                if(metadata->rows > 0 && metadata->cols > 0){
                                    //                                            std::string path = "/home/sigmind/Desktop/lp/" ;
                                    //                                            cv::imwrite(path+std::to_string(frame_meta->frame_num) +"_car"+ ".jpeg" ,*metadata_one);
                                    //                                            cv::imwrite(path+std::to_string(frame_meta->frame_num) +"_lp"+ ".jpeg" ,*metadata);
...
GstPadProbeReturn Deepstream_source_code::dsexample_buf_probe(GstPad *pad, GstPadProbeInfo *info, gpointer u_data)
{
    ...
    for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL;
            ...
            if (obj_meta->class_id == 0) {///Face for pgie_config_fd_lpd
                qDebug() << "face cond trueeee";
                ...
                for (NvDsMetaList * l_user_meta = obj_meta->obj_user_meta_list; l_user_meta != NULL;
                    ...
                    NvDsUserMeta * user_meta = (NvDsUserMeta *) (l_user_meta->data);
                    qDebug() << "user_meta->base_meta.meta_type face : "<<user_meta->base_meta.meta_type<<" "<<NVDS_USER_FRAME_META_EXAMPLE;
                    ...
                    if(user_meta->base_meta.meta_type == NVDS_USER_FRAME_META_EXAMPLE){
                        ...
                        cv::Mat *metadata_face = NULL;
                        metadata_face = (cv::Mat*) user_meta->user_meta_data;//sub object face/licence plate

                        std::string path = "/home/sigmind/Desktop/face/" ;
                        if(metadata_face->rows > 0 && metadata_face->cols > 0){
                            //                                            cv::imwrite(path+std::to_string(frame_meta->frame_num) +"_1"+ ".jpeg" ,*metadata_one);
                            //                                            cv::imwrite(path+std::to_string(frame_meta->frame_num)+"_2" + ".jpeg" ,*metadata);
                        ...
                        watchcam::rectangle rect(rectL,rectT,rectW,rectH);
                        rect.type = obj_meta->class_id;
                        qDebug()<<"rct typeee face"<<rect.type;
                        ...
                        //                                        qDebug()<<"metadata_one->rows : "<<metadata_one->rows<<" metadata_one->cols : "<<metadata_one->cols;
                        if(metadata_face->rows > 0 && metadata_face->cols > 0){
                            qDebug() << "beforee fromdstomain face";
                            MainWindow::fromDsToMain(metadata_face->clone(), metadata_face->clone(), rect);
                        }

                        try{
                            metadata_face->release();
                            metadata_face->deallocate();
                        }catch(...){}
                    }
                }
            ...
            else if (obj_meta->class_id == 1) {///LP for pgie_config_fd_lpd
                qDebug() << "lp cond truuu";
                ...
                for (NvDsMetaList * l_user_meta = obj_meta->obj_user_meta_list; l_user_meta != NULL;
                    ...
                    NvDsUserMeta * user_meta = (NvDsUserMeta *) (l_user_meta->data);
                    qDebug() << "user_meta->base_meta.meta_type lp : "<<user_meta->base_meta.meta_type<<" "<<NVDS_USER_FRAME_META_EXAMPLE;
                    ...
                    if(user_meta->base_meta.meta_type == NVDS_USER_FRAME_META_EXAMPLE){
                        ...
                        watchcam::rectangle rect(rectL,rectT,rectW,rectH);
                        rect.type = obj_meta->class_id;
                        qDebug()<<"rct typeee lp "<<rect.type;
                        ...
                        //                                        qDebug()<<"metadata_one->rows : "<<metadata_one->rows<<" metadata_one->cols : "<<metadata_one->cols;
                        if(metadata->rows > 0 && metadata->cols > 0){
                            qDebug() << "beforee fromdstomain lp";
                            MainWindow::fromDsToMain(metadata->clone(), metadata->clone(), rect);
                        }
                        //                                    MainWindow::fromDsToMain(metadata->clone(), rect);
